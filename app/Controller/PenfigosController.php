...
class PenfigosController extends AppController {
  ...
  public function diagnostico($idPaciente = null, $numero = null) {
    ...
    if (!empty($idPenfigo)) {
      ...
      if (!empty($idPenfigo)) {
        ...
        $diagnostico_t = $penfigos['resultado_sintomas'] + $penfigos['resultado_num_ampollas_m'];
        $penfigos['diagnostico'] = round($diagnostico_t / 2, 2);
      }
  ...
  public function get_num_ampollas($idPaciente = null, $numero = null, $idPenfigo = null, $tipo = null) {
    ...
    $array_i_aux = array();
    foreach ($ampollas_i as $key => $am) {
      $array_i_aux[$key]['Areaampolla.area_id'] = $am['Penfigoampolla']['area_id'];
      $array_i_aux[$key]['PacientesTipoampolla.tipoampolla_id'] = $am['Penfigoampolla']['tipoampolla_id'];
    }
    $array_a_i['OR'] = $array_i_aux;

    if (!empty($array_a_i) && !empty($array_i_aux)) {
    ...
    if (!empty($ampollas) && !empty($array_a_aux)) {
